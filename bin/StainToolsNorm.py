...
# Portions of this code adapted from StainTools
...
def get_concentrations(I, stain_matrix, regularizer=0.01):
    ...
    return np.linalg.lstsq(stain_matrix.T, OD.T, rcond=None)[0].T.clip(min=0)
...
class StainNormalizer(staintools.StainNormalizer):
    ...
    def transform(self, I, stain_matrix_source, maxC_source):
        ...
        tmp = np.dot(source_concentrations, self.stain_matrix_target)
        np.multiply(tmp, -1, out=tmp)
        np.exp(tmp, out=tmp)
        np.multiply(tmp, 255, out=tmp)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--canvasImagePath", type=str, required=True, help="canvas image path")
    ...
    max_color = 255
    quantile = 0.95
    v = max_color - int(np.quantile(img[::8, ::8, :].ravel(), quantile))
    print('Color max shift:', v)
    for c in range(img.shape[2]):
        print(f"Processing channel {c} with shift {v}...")
        img[..., c] = np.clip(img[..., c].astype(np.int16) + v, 0, max_color).astype(np.uint8)
    ...
    print(f"Prepared {len(coords)} patch coordinates for normalization.")

    if os.path.isfile(args.canvasImagePath):
        canvas = tifffile.imread(args.canvasImagePath)
        m, c = normalizer.estimate(canvas)
        print(f"Estimated stain matrix and maxC from canvas image: m={m}, c={c}")
    else:
        print('Canvas image not found, determining representative patch for normalization...')
