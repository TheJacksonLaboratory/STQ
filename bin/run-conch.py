...
if __name__ == '__main__':
    ...
    parser.add_argument('--use-conch-normalizer', dest='useCONCHnormalizer', action='store',
                        required=False, default=False,
                        help="""Use special normalization for CONCH, otherwise use the default normalizer""")
    ...
    useCONCHnormalizer = args.useCONCHnormalizer=='true'
    ...
    # Load pre-trained CONCH model
    model, normalizerCONCH = create_model_from_pretrained("conch_ViT-B-16", checkpoint_path=args.modelPath)
    model.eval()

    # Use special normalization for CONCH, otherwise use the default normalizer
    if useCONCHnormalizer:
        normalizer = normalizerCONCH
    ...
    for ibatch in tqdm(range(num_batches)):
        ...
        if len(images)>0:
            ...
            with torch.inference_mode():
                temp_features = model.encode_image(images, proj_contrast=False, normalize=False).cpu().numpy()
    ...
    df_features.columns = [f'feat_conch_' + str(i) for i in range(df_features.shape[1])]
