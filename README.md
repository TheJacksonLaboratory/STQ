...
The field of histopathology foundation models (FM) has grown rapidly, with new pretrained tile/slide encoders being released frequently (see [docs/foundation-models.txt](docs/foundation-models.txt) for the full list of models currently implemented in STQ, along with their release year, origin, and license). Rather than committing to a single embedding model, STQ is designed to let users extract and compare imaging features from many published foundation models side by side, using the same tiling grid, expansion factors, and downstream clustering/visualization steps. Most of these checkpoints are distributed under non-commercial or research-use-only licenses and are hosted on HuggingFace behind a gated-access agreement, so unlike the fully open Inception v3 model, users must individually register and request access on HuggingFace for each model of interest (e.g., UNI, UNI2, CONCH, Virchow, Virchow2, H-optimus-0/1, H0-mini, Phikon, Phikon-v2, prov-gigapath, prov-gigapath-flash, Kaiko-Midnight12k) and download the checkpoints into a private, user-only-readable cache directory before enabling the corresponding `extract_*_features` parameter. See [conf/models.config](conf/models.config) for the expected checkpoint paths and links to each model's HuggingFace page.
...
<details closed><summary>Click to see the commands used to upload singularity-built containers to quay.io</summary><p>
    ...
    singularity push /projects/chuang-lab/USERS/domans/containers/container-singularity-prov-gigapath-flash.sif oras://quay.io/jaxcompsci/provgigapathflash:v1.0.0
...
To download containers for use with the pipeline: change directory to the desirable download location and run the commands below:
    ...
    singularity pull oras://quay.io/jaxcompsci/provgigapathflash:v1.0.0
...
<details closed><summary>Click to get a template for conf/containers.config file:</summary><p>
    ...
    container_gigapathf        = "${params.container_dir}/provgigapathflash_v1.0.0.sif"
...
+ [DEPRECATED] If you plan to use StainNet within the pipeline, download the checkpoint from: 
...
6. **`Inception v3`** (https://huggingface.co/timm/inception_v3.tv_in1k)

Inception v3 is a convolutional neural network model developed and trained by Google on more than a million images from the ImageNet database. In our pipeline, the Inception v3 step is designed to take a whole slide image and grid information in the same format as generated by the SpaceRanger pipeline for a 10x Visium ST image. The 2048 imaging features (or mones) are generated for each image tile defined by the provided tiling grid. Starting with v0.4.0, the Inception v3 feature extractor was refactored from a Keras/TensorFlow implementation to a PyTorch/`timm` implementation, sharing the same unified feature-extraction script and container as the other foundation models below, which simplified maintenance and improved GPU batching performance.
...
7. **`CTransPath, MoCoV3, UNI, UNI2, CONCH, Virchow, Virchow2, H-optimus-0/1, H0-mini, Phikon, Phikon-v2, prov-gigapath, prov-gigapath-flash, Kaiko-Midnight12k`**

In addition to Inception v3, STQ implements a growing collection of histopathology foundation models as alternative tile-feature extractors, listed with release year, origin, and license in [docs/foundation-models.txt](docs/foundation-models.txt):
...
MoCoV3 (Facebook AI Research, 2021) is a self-supervised contrastive learning framework used as an alternative pretrained tile encoder.

UNI and UNI2 (https://huggingface.co/MahmoodLab/UNI, https://huggingface.co/MahmoodLab/UNI2-h) 
...
Virchow and Virchow2 (https://huggingface.co/paige-ai/Virchow, https://huggingface.co/paige-ai/Virchow2), developed by Paige-AI (now Tempus) and Microsoft Research, are tile-level foundation models trained on large proprietary histopathology slide collections.

H-optimus-0 and H-optimus-1 (https://huggingface.co/bioptimus/H-optimus-0, https://huggingface.co/bioptimus/H-optimus-1), and H0-mini (https://huggingface.co/bioptimus/H0-mini), are foundation models developed by Bioptimus (H0-mini jointly with Owkin).

Phikon and Phikon-v2 (https://huggingface.co/owkin/phikon, https://huggingface.co/owkin/phikon-v2), developed by Owkin, are self-supervised pathology foundation models distributed under an Owkin non-commercial license.

prov-gigapath and prov-gigapath-flash (https://huggingface.co/prov-gigapath/prov-gigapath, https://huggingface.co/prov-gigapath/prov-gigapath-flash), developed by University of Washington, Microsoft Research, and Providence Research, are research-use-only tile encoders; prov-gigapath-flash runs in its own dedicated `container_gigapathf` container (see [assets/container-singularity-prov-gigapath-flash.def](assets/container-singularity-prov-gigapath-flash.def)) due to its distinct dependency set.

Kaiko-Midnight12k (https://huggingface.co/kaiko-ai/midnight), developed by Kaiko.ai and released under the MIT license, is the only fully open-weight model among the recently added foundation models.

All of the above models (including the refactored Inception v3 extractor) share a single unified PyTorch/`timm`-based feature-extraction script and model-loading code path (`bin/run-extract.py`), which was optimized in v0.4.0 to reduce redundant model-loading overhead across large batches of tiles. Both UNI/UNI2 and CONCH, as well as most of the other gated models, require user registration and agreement to the use restrictions on HuggingFace; see [conf/models.config](conf/models.config) for the expected `pytorch_model.bin`/safetensors checkpoint paths, then adjust the pipeline parameters in [conf/analysis-img.config](conf/analysis-img.config) accordingly.
