...
include { GET_UNI2SUB_FEATURES;
          GET_CTRANSPATHSUB_FEATURES;
        } from '../modules/local/sub_features'
...
workflow SUBIMG {
    ...
    main:    
        ...
        if ( params.extract_tile_features ) {
           ...
           if (params.extract_uni2sub_features) {
                GET_UNI2SUB_FEATURES ( convertedimage
                                        .join(GET_TILE_MASK.out.mask)
                                        .join(TILE_WSI.out.grid)
                                        .join(LOAD_SAMPLE_INFO.out.grid)
                                        .join(imagesize)
                ...
                uni2sub_features_out = GET_UNI2SUB_FEATURES.out
            }

           if (params.extract_ctranspathsub_features) {
                GET_CTRANSPATHSUB_FEATURES ( convertedimage
                                        .join(GET_TILE_MASK.out.mask)
                                        .join(TILE_WSI.out.grid)
                                        .join(LOAD_SAMPLE_INFO.out.grid)
                                        .join(imagesize)
                ...
                ctranspathsub_features_out = GET_CTRANSPATHSUB_FEATURES.out
            }
            ...
            if (params.extract_uni2sub_features) {
                features_out = features_out.concat( uni2sub_features_out )
            }
            if (params.extract_conch_features) {
                features_out = features_out.concat( conch_features_out )
            }
            if (params.extract_ctranspathsub_features) {
                features_out = features_out.concat( ctranspathsub_features_out )
            }
